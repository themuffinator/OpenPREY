...
static const int FX_AXIS_CURRENT = 0;
static const int FX_AXIS_NORMAL = 1;
static const int FX_AXIS_BOUNCE = 2;
static const int FX_AXIS_INCOMING = 3;
static const int FX_AXIS_CUSTOMLOCAL = 4;

static const idMaterial *DeclFX_FindMaterial( const char *name, bool noCaching ) {
	return static_cast<const idMaterial *>( declManager->FindType( DECL_MATERIAL, name, true, noCaching ) );
}

static void DeclFX_PrecacheModel( const char *name, bool noCaching ) {
	if ( !noCaching ) {
		renderModelManager->FindModel( name );
	}
}

static int ParseUseAxisMode( const idToken &axisToken ) {
	if ( !axisToken.Icmp( "current" ) ) {
		return FX_AXIS_CURRENT;
	}
	if ( !axisToken.Icmp( "normal" ) ) {
		return FX_AXIS_NORMAL;
	}
	if ( !axisToken.Icmp( "bounce" ) ) {
		return FX_AXIS_BOUNCE;
	}
	if ( !axisToken.Icmp( "incoming" ) ) {
		return FX_AXIS_INCOMING;
	}
	if ( !axisToken.Icmp( "customlocal" ) ) {
		return FX_AXIS_CUSTOMLOCAL;
	}

	// Accept numeric enum values used by some shipped FX files.
	const char first = axisToken.Length() > 0 ? axisToken[0] : '\0';
	if ( ( first >= '0' && first <= '9' ) || first == '-' || first == '+' ) {
		return atoi( axisToken.c_str() );
	}
...
================
*/
void idDeclFX::ParseSingleFXAction( idLexer &src, idFXSingleAction& FXAction, bool noCaching ) {
	idToken token;
	...
	while (1) {
		...
		if ( !token.Icmp( "useAxis" ) ) {
			src.ReadToken( &token );
			FXAction.useAxis = ParseUseAxisMode( token );
			continue;
		}
		...
		if ( !token.Icmp( "angle" ) ) {
			...
			FXAction.dir = FXAction.axis[0];
			FXAction.explicitAxis = true;
			continue;
		}

		if ( !token.Icmp( "scale" ) ) {
			// Legacy Prey token used in a few FX files; this renderer path
			// does not consume it at runtime, but we parse and ignore it.
			src.ParseFloat();
		...
		if ( !token.Icmp( "uselight" ) ) {
			...
			DeclFX_FindMaterial( FXAction.data, noCaching );
			continue;
		}
		...
		if ( !token.Icmp( "attachentity" ) ) {
			...
			// precache the model
			DeclFX_PrecacheModel( FXAction.data, noCaching );
			continue;
		}
		...
		if ( !token.Icmp( "launch" ) ) {
			...
			declManager->FindType( DECL_ENTITYDEF, FXAction.data, true, noCaching );
			continue;
		}
		...
		if ( !token.Icmp( "sound" ) ) {
			...
			// precache it
			declManager->FindType( DECL_SOUND, FXAction.data, true, noCaching );
			continue;
		}
...
bool idDeclFX::Parse( const char *text, const int textLength, bool noCaching ) {
	...
	while( 1 ) {
		...
		if ( !token.Icmp( "{" ) ) {
			...
			ParseSingleFXAction( src, action, noCaching );
