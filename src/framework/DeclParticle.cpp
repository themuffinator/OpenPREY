...
================
*/
void idDeclParticle::ParseParametric( idLexer &src, idParticleParm *parm, bool noCaching ) {
	idToken token;
	bool negative = false;
	...
	if ( !token.Icmp( "-" ) ) {
		negative = true;
		if ( !src.ReadToken( &token ) ) {
			src.Error( "missing parameter after '-'" );
			return;
		}
	}
	...
	if ( token.IsNumeric() ) {
		...
		if ( src.ReadToken( &token ) ) {
			if ( !token.Icmp( "to" ) ) {
				...
				if ( !token.Icmp( "-" ) ) {
					negative = true;
					if ( !src.ReadToken( &token ) ) {
						src.Error( "missing second parameter after '-'" );
						return;
					}
				}
				parm->to = atof( token );
				...
				// Prey particle decls may append "with <curve>" after a range.
				if ( src.ReadToken( &token ) ) {
					if ( !token.Icmp( "with" ) ) {
						// Curve value is legacy metadata; consume for compatibility.
						src.ParseFloat();
					} else {
						src.UnreadToken( &token );
					}
				}
			} else if ( !token.Icmp( "with" ) ) {
				// Also allow "value with <curve>" forms.
				src.ParseFloat();
			} else {
				src.UnreadToken( &token );
			}
		...
		// table
		parm->table = static_cast<const idDeclTable *>( declManager->FindType( DECL_TABLE, token, false, noCaching ) );
	}
...
================
*/
idParticleStage *idDeclParticle::ParseParticleStage( idLexer &src, bool noCaching ) {
	idToken token;
	...
	while (1) {
		...
		if ( !token.Icmp( "material" ) ) {
			src.ReadToken( &token );
			stage->material = static_cast<const idMaterial *>( declManager->FindType( DECL_MATERIAL, token.c_str(), true, noCaching ) );
			continue;
		}
		if ( !token.Icmp( "count" ) ) {
		...
		if ( !token.Icmp( "speed" ) ) {
			ParseParametric( src, &stage->speed, noCaching );
			continue;
		}
		if ( !token.Icmp( "rotation" ) ) {
			ParseParametric( src, &stage->rotationSpeed, noCaching );
			continue;
		}
		if ( !token.Icmp( "angle" ) ) {
		...
		if ( !token.Icmp( "size" ) ) {
			ParseParametric( src, &stage->size, noCaching );
			continue;
		}
		if ( !token.Icmp( "aspect" ) ) {
			ParseParametric( src, &stage->aspect, noCaching );
			continue;
		}
		...
		if ( !token.Icmp( "lowSkippable" ) ) {
			// Legacy optimization hint from Prey tools, not needed by runtime simulation.
			src.ParseInt();
			continue;
		}
...
bool idDeclParticle::Parse( const char *text, const int textLength, bool noCaching ) {
	...
	while (1) {
		...
		if ( !token.Icmp( "{" ) ) {
			idParticleStage *stage = ParseParticleStage( src, noCaching );
			if ( !stage ) {
