...
cont:
	...
	uint32 *sourceVis, *passageVis, *portalVis, *mightSee, *prevMightSee, more;
	...
	for ( i = 0; i < area->numPortals; i++ ) {
		...
		prevMightSee = reinterpret_cast<uint32 *>(prevStack->mightSee);
		passageVis = reinterpret_cast<uint32 *>(passage->canSee);
		sourceVis = reinterpret_cast<uint32 *>(source->vis);
		mightSee = reinterpret_cast<uint32 *>(stack->mightSee);

		more = 0;
		// use the portal PVS if it has been calculated
		if ( p->done ) {
			portalVis = reinterpret_cast<uint32 *>(p->vis);
...
int idPVS::AreaPVSFromPortalPVS( void ) const {
	...
	for ( i = 0; i < numAreas; i++ ) {
		...
		for ( j = 1; j < area->numPortals; j++ ) {
			p1 = reinterpret_cast<uint32 *>(area->portals[0]->vis);
			p2 = reinterpret_cast<uint32 *>(area->portals[j]->vis);
...
void idPVS::Init( void ) {
	...
	areaVisLongs = areaVisBytes/sizeof(uint32);
	...
	portalVisLongs = portalVisBytes/sizeof(uint32);
...
pvsHandle_t idPVS::SetupCurrentPVS( const int *sourceAreas, const int numSourceAreas, const pvsType_t type ) const {
	...
	if ( type != PVS_CONNECTED_AREAS ) {
		...
		for ( i = 1; i < numSourceAreas; i++ ) {
			...
			vis = reinterpret_cast<uint32*>(areaPVS + sourceAreas[i] * areaVisBytes);
			pvs = reinterpret_cast<uint32*>(currentPVS[handle.i].pvs);
...
pvsHandle_t idPVS::MergeCurrentPVS( pvsHandle_t pvs1, pvsHandle_t pvs2 ) const {
	...
	ptr = reinterpret_cast<uint32*>(currentPVS[handle.i].pvs);
	pvs1Ptr = reinterpret_cast<uint32*>(currentPVS[pvs1.i].pvs);
	pvs2Ptr = reinterpret_cast<uint32*>(currentPVS[pvs2.i].pvs);
