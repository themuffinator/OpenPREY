...
def validate_shared_backend_contract() -> None:
    ...
    window_chrome = function_body(source, "static sdl3WindowChrome_t SDL3_GetWindowChrome(void) {")
    client_to_frame = function_body(source, "static void SDL3_GetWindowFrameFromClient(")
    frame_to_client = function_body(source, "static void SDL3_GetWindowClientPosition(")
    frame_display = function_body(source, "static SDL_DisplayID SDL3_GetDisplayForWindowFrame(")
    frame_intersection = function_body(source, "static bool SDL3_WindowFrameIntersectsAnyDisplay(")
    constrain_frame = function_body(source, "static void SDL3_ConstrainWindowFrameToBounds(")
    constrain_current = function_body(source, "static bool SDL3_ConstrainCurrentWindowFrameToDisplay(")
    ...
    require(window_chrome, "SDL_GetWindowBordersSize(s_sdlWindow", "cross-platform window chrome query")
    require(window_chrome, "SDL_WINDOW_BORDERLESS | SDL_WINDOW_FULLSCREEN", "zero-chrome window states")
    require(client_to_frame, "frame.x = clientX - chrome.left", "client-to-outer-frame position conversion")
    require(client_to_frame, "clientWidth + chrome.left + chrome.right", "client-to-outer-frame width conversion")
    require(frame_to_client, "clientX = frameX + chrome.left", "outer-frame-to-client position conversion")
    require(frame_display, "SDL_GetDisplayForRect(&frame)", "saved outer-frame display selection")
    require(frame_intersection, "clientWidth + chrome.left + chrome.right", "chrome-aware display intersection")
    require(constrain_frame, "bounds.w - horizontalChrome", "chrome-aware maximum client width")
    require(constrain_frame, "bounds.h - verticalChrome", "chrome-aware maximum client height")
    require(constrain_frame, "bounds.w - frameWidth", "complete-frame horizontal constraint")
    require(constrain_frame, "bounds.h - frameHeight", "complete-frame vertical constraint")
    require(constrain_current, "SDL3_GetDisplayWindowedPlacementBounds(display, bounds)", "post-decoration work-area reconciliation")
    require(constrain_current, "SDL_WINDOW_MINIMIZED | SDL_WINDOW_MAXIMIZED", "normal-frame preservation during fixed window states")
    require(constrain_current, "SDL3_GetWindowFrameFromClient", "live client-to-frame reconciliation")
    require(constrain_current, "SDL3_GetWindowClientPosition", "live frame-to-client reconciliation")
    ...
    require(screen_parms, "SDL3_WindowFrameIntersectsAnyDisplay", "chrome-aware off-screen recovery")
    require(screen_parms, "SDL3_GetDisplayForWindowFrame", "automatic restore to the saved frame's display")
    require(screen_parms, "SDL3_ConstrainWindowFrameToBounds", "chrome-aware window restore constraint")
    require(screen_parms, "SDL3_GetWindowClientPosition", "outer-frame restore conversion")
    require(screen_parms, "SDL3_ConstrainCurrentWindowFrameToDisplay", "post-show decoration reconciliation")
    require(screen_parms, 'SDL3_SyncWindowAfterScreenChange("window chrome reconciliation")', "post-decoration compositor sync")
    ...
    require(refresh_placement, "win32.win_xpos.SetInteger(frame.x)", "outer-frame window position persistence")
    require(refresh_placement, "!windowIsMinimized && !windowIsMaximized", "normal geometry persistence guard")
    require(refresh_placement, "isWindowedResizable", "borderless geometry persistence guard")
    ...
    require(window_event, "SDL3_ConstrainCurrentWindowFrameToDisplay", "display/scale chrome reconciliation")
    require(window_event, "SDL3_ResolveTargetDisplay(false).id", "explicit-display window constraint")
    require(window_event, "case SDL_EVENT_WINDOW_MAXIMIZED:", "maximized drawable refresh handling")
    ...
    require(display_event, "SDL3_ConstrainCurrentWindowFrameToDisplay", "SDL3 display work-area frame reconciliation")
    require(display_event, "SDL3_ResolveTargetDisplay(false).id", "SDL3 display-event target selection")
    ...
    require(create_window, "SDL3_WindowFrameIntersectsAnyDisplay", "initial chrome-aware off-screen recovery")
    require(create_window, "SDL3_GetDisplayForWindowFrame", "initial saved-frame display selection")
    require(create_window, "SDL3_ConstrainWindowFrameToBounds", "initial chrome-aware frame constraint")
    require(create_window, "SDL3_GetWindowClientPosition", "initial outer-frame placement conversion")
...
def validate_platform_wrappers() -> None:
    ...
    sdl_backend = read("src/sys/sdl3/sdl3_backend.cpp")
    ...
    require(sdl_backend, "SDL_PROP_WINDOW_COCOA_WINDOW_POINTER", "macOS native-window chrome fallback")
    require(sdl_backend, "Sys_MacOSGetWindowChrome", "shared macOS chrome fallback call")
    require(macos_compat, 'extern "C" bool Sys_MacOSGetWindowChrome', "macOS chrome fallback implementation")
    require(macos_compat, "contentRectForFrameRect", "macOS frame/content chrome calculation")
    require(macos_compat, "NSMaxY( frame ) - NSMaxY( content )", "macOS title-bar extent calculation")
