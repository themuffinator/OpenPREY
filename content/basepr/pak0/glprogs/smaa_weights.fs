...
float MaxSearchSteps() {
	return clamp( quality.z, 1.0, 32.0 );
}
...
void main() {
	...
	vec2 rtSize = max( texSize, vec2( 1.0, 1.0 ) );
	...
	if ( e.g > 0.0 ) {
		...
		vec2 sqrtD = sqrt( d );
		float rightEdge = texture( EdgesTex, coords.zy + vec2( invTexSize.x, 0.0 ) ).r;
	...
	if ( e.r > 0.0 ) {
		...
		vec2 sqrtD = sqrt( d );
		float bottomEdge = texture( EdgesTex, coords.xz + vec2( 0.0, invTexSize.y ) ).g;
