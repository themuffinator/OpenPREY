...
Status: proposed (2026-08-01). This is the execution-level companion to
[`openq4-rebase-plan.md`](openq4-rebase-plan.md) (strategy, standing decisions D1–D10,
risk register). Per-change evidence lives in the ledger under
[`docs-dev/openq4-rebase/`](openq4-rebase/) — every work package below cites ledger IDs
...
- **Ledger tracking:** when an item lands, append a line to its ledger entry:
  `PORTED: <commit-sha>` (or `DROPPED: <reason/commit>` for obsolete items when the
  decision is executed). The ledger is the audit trail for the next upstream sync.
...
- **Golden rule (cross-pollination):** wherever openPREY and upstream both implement
  something, take upstream's version and delete openPREY's. 31 ledger items are already
  dispositioned obsolete/drop; if you find an undocumented collision mid-port, prefer
  upstream and note it in the ledger.
- **ABI rules (apply to every phase):**
  1. Any change to `idRenderSystem`/`idRenderWorld`/`idMaterial` vtables or
     ...
     together. Batch such changes (WP4.1 is the single planned bump).
...
- Companion game repo: not required until WP3.1 chooses in-repo staging (D1 interim);
  if `../OpenPrey-game` exists, note its state — it is NOT the source of truth for this
  rebase (the in-tree `src/Prey`/`src/game` working tree is, per ledger narrative).
...
2. Grep-audit: `git grep -n "basepy"` — fix all remaining references (docs, scripts,
   `.vscode`, workflows). Keep a one-line note in `docs-dev/porting-baseline.md`.
...
4. Strings: move `*999.lang` packs into `content/basepr/pak0/strings/` unchanged for
   now (renumbering decision executes in WP7.4).
...
corelibs-22, corelibs-24, corelibs-25. Drops: corelibs-18, corelibs-23.
...
6. Drops: corelibs-18 (upstream ModelInfo), corelibs-23 (upstream AAS 1.07 handling —
   but ADD a validation note to Phase 8: load a retail Prey `.aas48` through upstream's
   path to confirm).
...
1. **Bring the Prey game trees onto the branch** exactly as committed at
   `pre-oq4-rebase`: `src/game`, `src/Prey`, `src/preyengine` (already present from
   WP2.1), shared header `src/framework/declPreyBeam.h` (already present from WP2.4).
   Interim model (D1): keep them in-repo; extend `tools/build/stage_gamelibs.py` with
   an "in-repo source root" mode (`OPENPREY_GAMELIBS_REPO` defaulting to the repo
   itself) staging `src/game` + `src/Prey` + `src/preyengine` into
   `.tmp/gamelibs_stage/` with the same manifest/hashing. This keeps upstream's
   configure-time gate and include-dir mechanism intact
   (`include_directories('.tmp/gamelibs_stage/src/game')` resolves
...
2. **Meson targets**: replace upstream's split `game-sp_/game-mp_` targets
   ...
   `basepy/meson.build` incl. per-file exclusions), linked against a single
...
3. Retire OLD's `sync_gamelibs.ps1`/`build_gamelibs.ps1` flow (upstream already
   no-ops sync; delete or leave the no-op).
...
- `gameImport_t` population: exactly the 13 Prey v7 systems (+ optional `hhProfiler*`
  under INGAME_PROFILER_ENABLED). No `.bse` member exists in Prey's struct — the
  in-tree BSE simply is not passed (D3).
...
- Add meson option `openprey_mvd=false` (or runtime cvar gate) compiling out / hard-
  disabling `MultiViewDemo` recording+playback entry points and bot spawn paths; every
  gated site gets a `// OPENPREY-GATED(D9):` marker comment so re-enablement is
  greppable.
...
(`spawnServer`) with the same module; `logs/openPREY_*.log` shows no contract-drift
fatals.

## 6. Phase 4 — renderer workstream

Ordering note: if WP3.x game compilation needs `renderEntity_s` fields, run WP4.1
between WP3.2 and WP3.3 — it is self-contained. The single-bump rule matters more than
phase numbering.

### WP4.1 — the ABI extension commit (RD-16 + RD-01/08 fields + RD-14 + RD-20 accessors)

One commit, `RENDER_API_VERSION` +1, all three binaries. Full inventory:
...
20-value `DEFORMTYPE_*` enum, `MAX_ENTITY_SHADER_PARMS 12→13` (+`EXP_REG_PARM12`
...
`referenceSound` decision (flagged in plan D2/RD row 11): choose here.
Default recommendation: keep upstream's int-handle model and adapt Prey game code's
refSound usage via a thin engine-side resolve helper — smaller ABI delta and keeps
upstream demo/MVD sound recording intact. If instead restoring `idSoundEmitter*`
(OLD's choice), audit upstream's demo/MVD sound paths in the same commit.

Vulkan side: add stubs returning defaults for every new virtual; renderer-vk must
still build (D2).

Acceptance: engine + renderer-gl + renderer-vk build; version handshake rejects a
stale module (test by rebuilding only the engine once — expect fail-closed GL load
error, then rebuild modules).
...
- ui-06: fonts path fallback `fonts/` (+ registration-order fix), `gui_smallFontLimit`
  ...
  under the TTF/bitmap dual pipeline with `r_ttf`-off default (D10).
...
- FW-24 savegames (conflict-major): implement as EXTENSIONS of upstream's hardened
  ...
  upstream's SAVEGAME_VERSION scheme (game payload stamps make OLD's 114 unnecessary —
  decide and record); multi-gamedir search (fs_game → basepr → base) via
...
`docs-dev/official-pk4-checksums.md` → new docs home (WP7.3).
...
- Adopt upstream's `docs/dev` + `docs/user` structure; port Prey docs (pk4 checksums,
  ...
  (companion staging doc points at the in-repo interim model per D1).
...
Decision: renumber openPREY's `#str_122xxx` menu labels onto upstream's
`english_openq4.lang` `#str_41xxx` advanced-rendering set (gaining upstream's larger
label coverage) — rebrand that file to `<lang>_openprey.lang` in
`content/basepr/pak0/strings/`; keep any Prey-only labels in the same file; update
`mainmenu_options.guifragment` bindings; delete `*999.lang`. (If retaining 122xxx
instead: move files into pak layout and record the decision — either is acceptable,
one must be chosen and noted in ledger assets-04.)
...
| Check | Command / lane |
|---|---|
| Engine-only build | `meson setup builddir-engine -Dbuild_games=false && meson compile -C builddir-engine` |
| Full build (Win x64) | `tools/build/meson_setup.ps1 compile -C builddir` |
| Linux build | upstream commit-validation lane (WP7.1) |
| renderer-vk builds w/ Prey stubs | part of full build |
| ABI handshake | intentionally stale renderer module → fail-closed GL error |
| Module load diagnostics | launch with `com_logGameModuleLoad`-equivalent; all phases green |
| Event dispatch x64 (WP6.3) | scripted event smoke on `roadhouse_quick` |
| Pak validation | startup against retail digital layout + classic layout (presence-only) |

### WP8.2 — runtime matrix (retail assets)

1. Install auto-detection: launch from a neutral cwd; registry discovery finds retail;
   `fs_basepath` correct in log.
2. SP campaign starts: `roadhouse`, `feedingtowera`, a spiritwalk map, a shuttle map,
   `girlfriendx` (portals), each via the generated launch configs; play 2–3 minutes.
3. Renderer parity spot-checks (per Phase 4 gate list) + corona/lens-flare
   double-draw check + lightgrid bake of one map via upstream `bakeLightGrids`.
4. Sound: meters attenuation A/B (WP5.4), subtitles, censor, music volume class,
   menu device switch.
5. Saves: save/load round-trip SP; quickload prompt; load from `base/` vs `basepr/`;
   config casing migration (drop an `OpenPreyConfig.cfg` into savepath — must load
   once and re-write as `openPREYConfig.cfg`).
6. MP: listen server + second client join, Deathmatch on `dmtopillogical_4`;
   usercmd layout sanity (alt-fire works over network).
7. Startup-warning audit under stock assets (TODO item): capture
   `logs/openPREY_*.log`, triage every warning into fix/accept lists.
8. `.cm` fixture verification (assets-06): regenerate one retail map's collision via
   the rebased engine; diff against the committed mirror.
9. Known-gap confirmation: mirror clip plane workaround still active + tracked
   (`references/retail_mirror_clip_parity_plan.md` untouched by rebase).

### WP8.3 — cut-over

1. Ledger sweep: every one of the 137 IDs has `PORTED:`/`DROPPED:` (or an explicit
   `DEFERRED:` with a TODO entry — expected deferrals: ui-02 TTF variant, MVD/bots
   re-enable, sound DSP effects, ModernGL/Vulkan Prey parity, classic-pak checksums,
   mirror clip math).
2. Merge/fast-forward `new-prey` ← `prey-on-oq4` (or retarget the default branch);
   keep `pre-oq4-rebase` tag permanently.
...
| corelibs-23 | obsolete-upstream | — (AAS load check in 8.2) |
...
| ui-02 spline text under TTF | WP5.8 note; TTF default-off (D10) |
...
| Companion-repo migration (OpenPrey-game) | D1 interim → full staging later |
